...
usrp2_impl::usrp2_impl(const device_addr_t &_device_addr){
    ...
    for (size_t mbi = 0; mbi < device_args.size(); mbi++){
		...
		// N210 FPGA's custom 100 Msps DSP chain is DSP 1.
		// The UHD_SWAP_RX_DSPS environment variable swaps the DSPs.
		// Someday this might be easier.
		//
        std::string	dsp_port0 = 		BOOST_STRINGIZE(USRP2_UDP_RX_DSP0_PORT);
        std::string	dsp_port1 = 		BOOST_STRINGIZE(USRP2_UDP_RX_DSP1_PORT);
        size_t	settingsRegRx0 = 		U2_REG_SR_ADDR(SR_RX_DSP0);
        size_t	settingsRegRx1 = 		U2_REG_SR_ADDR(SR_RX_DSP1);
        size_t	settingsRegControl0 = 	U2_REG_SR_ADDR(SR_RX_CTRL0);
        size_t	settingsRegControl1 = 	U2_REG_SR_ADDR(SR_RX_CTRL1);
      	boost::uint32_t	rxSidBase0 = 	USRP2_RX_SID_BASE + 0;
      	boost::uint32_t	rxSidBase1 = 	USRP2_RX_SID_BASE + 1;
      	
    	const char *swapDsps = getenv("UHD_SWAP_RX_DSPS");
        if (swapDsps && atoi(swapDsps))
        {
        	UHD_LOG << "Swapping rx DSPs..." << std::endl;
        	std::swap(dsp_port0, dsp_port1);
        	std::swap(settingsRegRx0, settingsRegRx1);
        	std::swap(settingsRegControl0, settingsRegControl1);
        	std::swap(rxSidBase0, rxSidBase1);
        }
        
        UHD_LOG << "Making transport for RX DSP0..." << std::endl;
        _mbc[mb].rx_dsp_xports.push_back(make_xport(
            addr, dsp_port0, device_args_i, "recv"
        ));
        UHD_LOG << "Making transport for RX DSP1..." << std::endl;
        _mbc[mb].rx_dsp_xports.push_back(make_xport(
            addr, dsp_port1, device_args_i, "recv"
        ));
        ...
        _mbc[mb].rx_dsps.push_back(rx_dsp_core_200::make(
            _mbc[mb].iface, settingsRegRx0, settingsRegControl0, rxSidBase0, true
        ));
        _mbc[mb].rx_dsps.push_back(rx_dsp_core_200::make(
            _mbc[mb].iface, settingsRegRx1, settingsRegControl1, rxSidBase1, true
        ));
